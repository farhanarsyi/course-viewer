...
function renderLesson() {
  ...
  if (allVideos.length > 0) {
    ...
    `).join('') + `
      <a id="external-video-link" href="${escapeHtml(allVideos[state.currentVideoIdx].url)}" target="_blank" rel="noopener noreferrer" class="video-tab-btn" style="text-decoration:none; color:var(--accent); border-color:var(--accent); margin-left:auto;">
...
function buildVideoEmbed(vid) {
  ...
  if (isCdn) {
    // Route b-cdn.net videos through our new Cloudflare Worker proxy
    videoSrc = `https://video-proxy.farhanarsyi.workers.dev/?url=${encodeURIComponent(videoSrc)}`;
  }
